...
func (s *LLMDeploymentService) DeployLLMProviderConfiguration(params LLMDeploymentParams) (*APIDeploymentResult, error) {
	...
	apiCfgPtr := s.transformer.Transform(&providerConfig, &apiConfig)
	if apiCfgPtr == nil {
	...
	storedCfg := &models.StoredConfig{
		ID:                  apiID,
		Kind:                string(api.Llmprovider), // mark kind as llm/provider
	...
	// Log
	msg := "LLM provider configuration created"
	if isUpdate {
		msg = "LLM provider configuration updated"
	}
	params.Logger.Info(msg,
...
func (s *LLMDeploymentService) DeleteLLMProvider(name, version, correlationID string, logger *zap.Logger) error {
	cfg, err := s.store.GetByNameVersion(name, version)
	if err != nil {
		return fmt.Errorf("LLM provider configuration with name '%s' and version '%s' not found", name, version)
	}
	if s.db != nil {
	...
	if err := s.store.Delete(cfg.ID); err != nil {
		return fmt.Errorf("failed to delete configuration from memory store: %w", err)
	}
	go func() {
		ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
		defer cancel()
...
func (s *LLMDeploymentService) CreateLLMProviderTemplate(params LLMTemplateParams) (*models.StoredLLMProviderTemplate, error) {
	...
	if len(validationErrors) > 0 {
		errs := make([]string, 0, len(validationErrors))
		params.Logger.Warn("Template validation failed", zap.String("name", tmpl.Data.Name), zap.Int("error_count", len(validationErrors)))
		for i, e := range validationErrors {
			params.Logger.Warn("Validation error", zap.String("field", e.Field), zap.String("message", e.Message))
			errs = append(errs, fmt.Sprintf("%d. %s: %s", i+1, e.Field, e.Message))
		}
		...
		if sqlite, ok := s.db.(*storage.SQLiteStorage); ok {
			if err := sqlite.SaveLLMProviderTemplate(stored); err != nil {
				if storage.IsConflictError(err) || strings.Contains(err.Error(), "already exists") {
					return nil, fmt.Errorf("template with name '%s' already exists", tmpl.Data.Name)
				}
...
// GetLLMProviderByNameVersion returns a stored LLM provider by name and version
func (s *LLMDeploymentService) GetLLMProviderByNameVersion(name, version string) (*models.StoredConfig, error) {
	cfg, err := s.store.GetByNameVersion(name, version)
	if err != nil {
		return nil, err
	}
	if cfg.Kind != string(api.Llmprovider) {
		return nil, fmt.Errorf("configuration with name '%s' and version '%s' is not an LLM provider", name, version)
	}
	return cfg, nil
}
...
func (s *LLMDeploymentService) UpdateLLMProvider(name, version string, params LLMDeploymentParams) (*models.StoredConfig, error) {
	...
	if existing.Kind != string(api.Llmprovider) {
		return nil, fmt.Errorf("configuration with name '%s' and version '%s' is not an LLM provider", name, version)
	}

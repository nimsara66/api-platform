...
func (t *LLMProviderTransformer) Transform(config interface{}) (*api.APIConfiguration, error) {
	provider, ok := config.(*api.LLMProvider)
	if !ok {
		return nil, fmt.Errorf("llm provider transformer expected *api.LLMProvider, got %T", config)
	}

	// 1) Retrieve the referenced template from in-memory
	tmpl, err := t.store.GetTemplateByName(provider.Data.Template)
	if err != nil {
		return nil, fmt.Errorf("template '%s' not found: %w", provider.Data.Template, err)
	}
	...
	apiCfg.Data.Upstream = make([]api.Upstream, 0, len(provider.Data.Upstreams))
	for _, u := range provider.Data.Upstreams {
		apiCfg.Data.Upstream = append(apiCfg.Data.Upstream, api.Upstream{Url: u.Url})
	}

	// 3b) Apply access control (filter operations)
	applyAccessControl(provider.Data.AccessControl, &apiCfg.Data.Operations)

	// 3c) Attach policies from provider to relevant operations
	attachPolicies(provider.Data.Policies, &apiCfg.Data.Operations)

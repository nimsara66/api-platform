...
func TestComponentVersion(t *testing.T) {
	definition := &components.Definition{Name: "platform-gateway"}
	tests := []struct {
		name     string
		topology *Topology
		want     string
		...
		{name: "resolved version", topology: &Topology{Block: &topology.ResolvedBlock{
			Name:       "gateway-core",
			Components: []topology.ResolvedComponent{{Def: definition, Version: "1.2.0-SNAPSHOT"}},
		}}, want: "1.2.0-SNAPSHOT"},
		{name: "missing version", topology: &Topology{Block: &topology.ResolvedBlock{
			Name:       "gateway-core",
			Components: []topology.ResolvedComponent{{Def: definition}},
		}}, wantErr: "no resolved version"},
		{name: "missing component", topology: &Topology{Block: &topology.ResolvedBlock{Name: "gateway-core"}}, wantErr: "not declared"},
		{name: "missing block", topology: &Topology{}, wantErr: "block is required"},
		{name: "nil topology", wantErr: "topology is required"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := tt.topology.ComponentVersion("platform-gateway")
			if tt.wantErr != "" {

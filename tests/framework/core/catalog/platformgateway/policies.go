...
func BuildSourceWithPolicies(
	...
	spec, err := sourcePolicyBuildSpec(version, workspace)
	if err != nil {
		return DerivedImages{}, err
	}
	...
	return DerivedImages{
		Controller: "ghcr.io/wso2/api-platform/gateway-controller:" + version,
		Runtime:    "ghcr.io/wso2/api-platform/gateway-runtime:" + version,
	}, nil
}
...
func sourcePolicyBuildSpec(version string, workspace policyWorkspace) (builder.Spec, error) {
	...
	return builder.Spec{
		...
		Plan: func(root, v string, coverage builder.CoverageSpec) ([]builder.Command, error) {
			return sourcePolicyBuildCommands(root, v, workspace, controller, runtime, coverage), nil
		},
	}, nil
}

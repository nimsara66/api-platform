...
func TestBuildSourceWithPoliciesStagesCompletePolicyTree(t *testing.T) {
	root := unitRepoRoot(t)
	source := filepath.Join("..", "gateway-controllers", "policies")
	...
	require.Equal(t, "ghcr.io/wso2/api-platform/gateway-controller:1.2.0-SNAPSHOT", images.Controller)
	require.Equal(t, "ghcr.io/wso2/api-platform/gateway-runtime:1.2.0-SNAPSHOT", images.Runtime)
...
func TestStagePolicyWorkspaceGeneratesDeterministicManifest(t *testing.T) {
	root := unitRepoRoot(t)
	source := filepath.Join(root, ".framework-policy-source-fixture")
	...
	workspace, err := stagePolicyWorkspace(root, ".framework-policy-source-fixture")
	require.NoError(t, err)
...
func TestPolicyWorkspaceRejectsDuplicatePolicyNames(t *testing.T) {
	root := unitRepoRoot(t)
	source := filepath.Join(root, ".framework-policy-duplicate-")
	require.NoError(t, os.RemoveAll(source))
	t.Cleanup(func() { require.NoError(t, os.RemoveAll(source)) })
	...
	_, err := BuildSourceWithPolicies(context.Background(), root, "1.2.0-SNAPSHOT", ".framework-policy-duplicate-", &policyRecordingRunner{}, false)
...
func TestVersionedPolicyBuildDoesNotReturnImagesAfterCommandFailure(t *testing.T) {
	...
	images, err := BuildVersionedWithPolicies(context.Background(), root, "1.2.0-SNAPSHOT",
		filepath.Join("..", "gateway-controllers", "policies"),
		"ghcr.io/wso2/api-platform/gateway-controller:1.2.0-SNAPSHOT",
		"ghcr.io/wso2/api-platform/gateway-runtime:1.2.0-SNAPSHOT", runner)

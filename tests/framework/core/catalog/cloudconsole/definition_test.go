...
func TestLoadEnvironmentConfig(t *testing.T) {
	actual, err := LoadEnvironmentConfig("environments.toml")
	require.NoError(t, err)
	require.Equal(t, "dev.cloud.wso2.com", actual.Environments["development"].CloudBaseDomain)
...
func TestResolveEndpoints(t *testing.T) {
	...
	endpoints, err := resolveEndpoints(root, map[string]string{EnvironmentParameter: "dev"})
	require.NoError(t, err)
	require.Equal(t,
		"https://dev-wso2cloud.gateway.dev.example.com/apip-bml-apip-bml-endpoint/api/v0.9",

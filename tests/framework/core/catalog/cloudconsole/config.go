...
func LoadEnvironmentConfig(path string) (EnvironmentConfig, error) {
	...
	for name, rawEntry := range environments {
		if strings.TrimSpace(name) == "" {
			return EnvironmentConfig{}, fmt.Errorf("cloud-console: environment config %q contains an empty environment name", path)
		}
...
func (c EnvironmentConfig) Domain(environment string) (string, error) {
	...
	if environment == "" {
		return "", fmt.Errorf("cloud-console: environment is required")
	}

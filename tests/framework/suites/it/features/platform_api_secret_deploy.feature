...
Feature: Platform-API-driven deployment resolves secret references on demand
  ...
  # This exercises platform-api's own artifact-import/deployment pipeline - a resource is
  # created and deployed through platform-api's REST API, not gateway-controller's - which is
  # a genuinely different mechanism from secrets.feature (gateway-controller-direct). Each
  # scenario deploys without restarting the controller, so the assertion exercises the
  # on-demand *.deployed event path (the controller resolves {{ secret "..." }} at deploy
  # time) rather than the startup bulk-sync path. The gateway is already registered with the
  # control plane under the fixed handle "it-gateway" (see
  # core/catalog/platformapi's provisionGatewayRegistration).

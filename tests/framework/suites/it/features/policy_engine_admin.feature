...
Feature: Policy engine admin API
  ...
  # config_dump for policy_engine.admin is already enabled globally by the framework's existing
  # shared gateway-controller-storage.toml overlay - no dedicated overlay/block is needed. The
  # generic service-request step already waits for dump consistency with most create/delete this
  # scenario performed (see awaitDumpConsistent), except for a route deletion, which needs its
  # own much longer wait - see the known-issue scenario below.
  ...
  # Known product-contract gap: gateway-controller's policyxds.SnapshotManager.UpdateSnapshot
  # (pkg/policyxds/snapshot.go) pushes a deletion marker via UpdateResource before SetResources
  # for the event-channel cache, to work around go-control-plane's LinearCache not always
  # notifying SotW wildcard watches of a pure deletion - but the policy chain and route config
  # caches just above it call SetResources directly, with no equivalent marker. A route deletion
  # can therefore go unnotified to the policy engine's ADS watch until an unrelated resource
  # change on the same cache happens to force a fresh push, which is consistent with what was
  # observed here: propagation time varied between ~70s and over 240s across otherwise-identical
  # runs rather than converging on a bounded delay. Excluded from the runner via it-suite.yaml's
  # per-runner "~@known-issue" tag until the policy engine controller applies the same
  # deletion-marker workaround to the policy chain and route config caches.
  @known-issue

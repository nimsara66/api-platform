...
Feature: Advanced rate limit key extraction strategies
  ...
  # A route dropping a shared quota must decrement its reference count, not delete the
  # bucket outright - otherwise a sibling route still using that quota loses its accumulated
  # state on the next reconfiguration. This scenario was written before the engine's
  # ref-counted cleanup fix shipped (advanced-ratelimit v1.1.3+) and was disabled until then;
  # verified against the current build (v1.2.0) that the fix is in place.

...
Feature: Azure Content Safety content moderation policy
  ...
  # The endpoint/key are gateway-wide systemParameters for this policy (no per-attachment
  # override exists), so they are supplied via the azure-content-safety block's config
  # overlay (core/catalog/overlays/azure-content-safety.toml), pointing at the testbench
  # "content-safety" mock. The mock flags text containing configured keywords per category
  # (hate/violence/sexual/self-harm) and requires the exact subscription key the overlay sets.
  ...
  # Legacy skipped response-phase validation entirely, noting its mock backend could not be
  # made to return violating content on demand. The echo testbench backend reflects whatever
  # the client sends, so the echoed body IS controllable content for the response phase to
  # inspect - closing that gap rather than repeating the omission.

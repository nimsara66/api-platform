...
Feature: Prompt template
  ...
  # The upstream is the echo testbench service, which reflects the request it received back as
  # its own response ("json"/"data" fields). Since prompt-template rewrites the request body
  # before it reaches the upstream, the echoed body proves the substitution actually happened,
  # rather than only that the request did not fail.

...
Feature: AWS Bedrock guardrail policy
  ...
  # The gateway's AWS Bedrock client is redirected at the testbench "bedrock" mock
  # (AWS_ENDPOINT_URL_BEDROCK_RUNTIME) by default, so no per-scenario endpoint/credential
  # configuration is needed - only the policy's request/response params. The mock recognizes
  # "violence"/"hate"/"illegal" as content-policy violations, an "@example.com" address as PII
  # (masked-then-restored when prefixed "mask-", permanently redacted otherwise), and
  # "error"+"simulate" together as a simulated upstream 500.

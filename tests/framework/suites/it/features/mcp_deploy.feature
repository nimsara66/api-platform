...
Feature: MCP proxy CRUD and connectivity
  ...
  # Legacy carried a regression test here: an MCP proxy with no policies attached, fronting a
  # streamable-HTTP server whose responses combine a Content-Length header with chunked
  # transfer-encoding on the same SSE body, used to return HTTP 500 from Envoy
  # ("mismatch_between_content_length_and_the_length_of_the_mutated_body"). That upstream was
  # the real @modelcontextprotocol/server-everything npm package; reproducing its exact wire
  # behavior in the testbench (without adding a second, non-testbench mock container, which
  # this framework's conventions disallow) was attempted via a hand-crafted raw-socket
  # response carrying both headers, but Envoy's own upstream HTTP/1.1 codec rejects that
  # combination outright as a protocol error before any response processing begins - a
  # different, more severe failure than the subtler mismatch legacy's real server produced.
  # This scenario is narrowed to cover the notifications/initialized and tools/list client
  # steps against the well-behaved testbench MCP server instead; the original wire-level
  # regression is not currently reproduced - see task backlog for a follow-up decision.

...
Feature: Analytics basic event capture
  ...
  # The framework's own gateway-analytics.toml (applied to every block automatically) already
  # points the moesif publisher at this block's own testbench analytics partition. This block
  # runs alone (parallel: 1) so no other runner's traffic can inflate this feature's counts.

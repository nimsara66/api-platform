...
Feature: Semantic tool filtering policy
  ...
  # the same semantic-ai block overlay used by semantic-cache and semantic-prompt-guard. The
  # echo upstream reflects the request body back, so the "tools" array visible in the response
  # is exactly what the policy passed through after filtering.

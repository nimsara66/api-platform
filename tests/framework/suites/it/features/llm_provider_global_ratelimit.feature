...
Feature: Provider-wide rate limiting for LLM providers and proxies
  ...
  Scenario: deprecated policies field still enforces independent per-resource rate limits
    # Identical to the operationPolicies scenario above but using the legacy "policies" field -
    # a regression guard, since "policies" is deprecated but still honoured identically to
    # operationPolicies.

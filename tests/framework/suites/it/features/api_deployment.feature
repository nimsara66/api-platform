...
Feature: Platform-API-driven API deployment is served by the gateway data plane
  ...
  # Unlike platform_api_secret_deploy.feature, which only confirms gateway-controller's
  # management API reports an artifact as configured, this exercises the actual data-plane
  # request path through gateway-runtime - proving a platform-api-driven deployment results
  # in real, working traffic, not just a correctly-populated configuration record.

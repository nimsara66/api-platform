...
Feature: MCP proxy behavior under attached policies
  ...
  # The mcp-auth/mcp-authz policies validate JWTs against a gateway-wide keymanager registry
  # (systemParameters.keyManagers), not a per-attachment parameter - supplied via this block's
  # mcp-jwt-auth.toml overlay, which registers keymanager "mock-jwks" with its issuer/jwks URIs
  # pointing at the testbench jwks mock. Tokens are minted directly from that same mock via the
  # "I get a JWT token..." steps below.
  ...
  # mcp-auth exposes jwt-auth's token-forwarding parameters (forwardToken, forwardedTokenHeader,
  # forwardTokenStripScheme, userIdClaim). The testbench MCP mock does not echo request headers,
  # so what actually reaches the upstream under forwardedTokenHeader is a policy unit-test
  # concern; here we only guard config acceptance and that forwarding does not break auth.

...
Feature: JWT authentication
  ...
  # The jwt-auth policy validates tokens against a gateway-wide keymanager registry
  # (systemParameters.keyManagers), supplied by this block's mcp-jwt-auth.toml overlay - the
  # same "mock-jwks" and "wrong-issuer-km" keymanagers already used by mcp-auth/mcp-authz
  # tests, since both policies share the identical jwtauth_v1 configuration.
  ...
  # The testbench jwks /token endpoint always mints "test-audience" as the aud claim (see
  # testbench/services/jwks), so an operation expecting a DIFFERENT audience proves rejection.

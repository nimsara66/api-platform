...
Feature: Template functions in resource specs
  ...
  # IT_TEMPLATE_PATH, IT_RATE_LIMIT and IT_ALLOW_CREDENTIALS are set unconditionally on the
  # gateway-controller container (see core/catalog/platformgateway/docker-compose.yaml) for
  # exactly these scenarios; IT_DEFINITELY_MISSING_KEY is deliberately never set, to exercise
  ...
  Scenario: env template in integer policy param is coerced and enforced at runtime
    # IT_RATE_LIMIT=5 is set in platformgateway/docker-compose.yaml. The template renders to
    # the string "5"; the controller coerces it back to the numeric value 5 so schema
    # validation passes (type: integer) and the policy engine enforces the correct limit.
  ...
  Scenario: env template in boolean policy param is coerced and applied at runtime
    # IT_ALLOW_CREDENTIALS=true is set in platformgateway/docker-compose.yaml. The template
    # renders to the string "true"; the controller coerces it back to bool(true) so schema
    # validation passes (type: boolean) and the CORS policy emits the credentials header.

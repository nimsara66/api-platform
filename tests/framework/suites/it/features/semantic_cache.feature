...
Feature: Semantic cache policy
  ...
  # embeddingProvider/embeddingEndpoint/vectorStoreProvider/dbHost/dbPort are gateway-wide
  # systemParameters (no per-attachment override), supplied via the semantic-cache block's
  # config overlay pointing at the testbench "embeddings" mock (OpenAI-compatible, deterministic
  # word-hash embeddings - see testbench/services/embeddings) and the block's own "redis" component.

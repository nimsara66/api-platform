...
Feature: Subscription analytics monetized billing metadata
  ...
  # Calls gateway-controller's own POST /subscriptions REST API directly rather than the
  # legacy mock-platform-api "inject-subscription" indirection: gateway-controller's endpoint
  # accepts the identical subscriptionPlanId/billingCustomerId/billingSubscriptionId fields and
  # is real product code, and no real platform-api path produces these billing fields at all.

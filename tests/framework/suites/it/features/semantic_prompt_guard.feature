...
Feature: Semantic prompt guard policy
  ...
  # embeddingProvider/embeddingEndpoint/apiKey are gateway-wide systemParameters supplied via
  # the semantic-ai block's config overlay pointing at the testbench "embeddings" mock. Unlike
  # semantic-cache, this policy has no vector store - similarity is computed and compared
  # synchronously within the request, so no propagation-race handling is needed here.

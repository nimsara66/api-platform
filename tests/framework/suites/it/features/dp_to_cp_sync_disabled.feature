...
Feature: Data-plane to control-plane push is suppressed when deployment sync is disabled
  ...
  # This runner's block applies dp-to-cp-sync-disabled.toml, which sets
  # controller.controlplane.deployment_sync_enabled=false. The gateway still connects to the
  # control plane, but must not push gateway-originated artifacts up - they stay local only.
  # See dp_to_cp.feature for the sync-enabled behaviour these scenarios are the negative of.

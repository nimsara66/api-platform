...
Feature: Configuration dump endpoint
  ...
  # config_dump is off by default in production; the framework's own shared
  # gateway-controller-storage.toml overlay (applied to every block unconditionally) already
  # enables it for both controller.admin_server and policy_engine.admin, so no dedicated
  # overlay or block is needed for this feature specifically.

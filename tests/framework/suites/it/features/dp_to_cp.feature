...
Feature: Data-plane to control-plane artifact push
  ...
  # The gateway-controller pushes every gateway-originated artifact (LLM provider template
  # / provider / proxy, MCP proxy, REST API) up to the control plane on create and update,
  # tells it to undeploy on delete, and re-pushes any failed artifact on reconnect. This is
  # gated on deployment_sync_enabled (default true, see dp_to_cp_sync_disabled.feature for
  # the disabled case) and an active control-plane connection.
  #
  # The control plane here is the real platform-api product (core/catalog/platformapi), not
  # a recorder - every assertion below is a real read against platform-api's own management
  # API. MCP/RestApi/LlmProxy are project-scoped on the control plane (see
  # internal/service/artifact_import.go's RequiresProject), so each such scenario creates a
  # real project first and tags the artifact with its handle via the
  # gateway.api-platform.wso2.com/project-id annotation.
  ...
  Scenario: A push rejected by the control plane is recorded as failed and re-pushed on reconnect
    ...
    # The project-id annotation names a project that does not exist yet on the control plane,
    # so platform-api's own import validation genuinely rejects this push (ProjectNotFound) -
    # a real product rejection, not a test-only toggle.
    When I create MCP proxy from "resources/templates/mcp.yaml" with values:
      | apiVersion            | gateway.api-platform.wso2.com/v1 |
      | name                  | ${CTX:mcpName}                    |
      | displayName           | ${CTX:mcpDisplayName}             |

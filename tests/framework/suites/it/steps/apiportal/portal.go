...
func (s *Steps) waitForWebhookEventStatus(ctx context.Context, eventID, wantStatus string) error {
	...
	if err != nil {
		return fmt.Errorf("waiting for API Portal webhook event %q to reach %q: %w", eventID, wantStatus, err)
	}
	if !result {
		return fmt.Errorf("API Portal webhook event %q did not reach status %q before the timeout", eventID, wantStatus)
	}
	return nil
}

...
func (s *Steps) findActiveGateway(ctx context.Context) error {
	...
	for _, item := range items {
		...
		return tcontext.Set(ctx, keyGatewayEndpoint, strings.TrimRight(endpoint, "/"))
	}
...
func (s *Steps) sendUntilStatus(ctx context.Context, method, target string, want int) error {
	...
	target, err = resolveCloudDataPlaneURL(ctx, target)
	if err != nil {
		return err
	}
...
func resolveCloudDataPlaneURL(_ context.Context, target string) (string, error) {
	...
	if (parsed.Scheme != "http" && parsed.Scheme != "https") || parsed.Host == "" {
		return "", fmt.Errorf("cloud data-plane target must be an absolute HTTP(S) URL, got %q", target)
	}
	return target, nil
}

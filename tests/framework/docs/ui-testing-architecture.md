...
Source-built Go services may expose `GOCOVERDIR` data without product HTTP endpoints. Node
services use `NODE_V8_COVERAGE`; the framework collects both after graceful shutdown. Frontend
coverage is collected from instrumented Chromium pages through Playwright, stored per scenario,
and converted separately to Istanbul JSON, LCOV, and HTML. Browser coverage is not mixed with
Go or Node-server coverage.

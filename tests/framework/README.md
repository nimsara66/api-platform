...
mapping is maintained in `core/catalog/cloudconsole/environments.toml`; add or update an
environment there without changing Go code. The CI workflow is responsible for obtaining the
cloud access token; the suite consumes it from `CLOUD_CONSOLE_ACCESS_TOKEN` and never calls the
token provider itself. Its suite defaults match the integration suite: a 5-minute boot deadline
